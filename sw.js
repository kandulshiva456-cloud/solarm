...
self.addEventListener('fetch', event => {
  ...
  // HTML stays fresh. JavaScript/CSS are network-first so a GitHub Pages
  // deployment cannot leave the site running an older gallery implementation.
  if (event.request.mode === 'navigate' || url.pathname.endsWith('.html') || /\.(?:css|js)$/.test(url.pathname)) {
    event.respondWith(
      fetch(event.request)
        .then(response => {
